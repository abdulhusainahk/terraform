...
output "state_machine_creation_date" {
  ...
  value       = try(aws_sfn_state_machine.sfn_state_machine[0].creation_date, "")
}

output "state_machine_status" {
  description = "The current status of the Step Function"
  value       = try(aws_sfn_state_machine.sfn_state_machine[0].status, "")
}

# IAM Role
output "role_arn" {
  description = "The ARN of the IAM role created for the Step Function"
  value       = try(aws_iam_role.state_machine_role[0].arn, "")
}

output "role_name" {
  description = "The name of the IAM role created for the Step Function"
  value       = try(aws_iam_role.state_machine_role[0].name, "")
}